...
export function Projects() {
  ...
  return <section id="projects" className="section bg-white dark:bg-portfolio-primary">
      <div className="container px-4 mx-[20px]">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {projects.map(project => <div key={project.id} className="reveal relative group overflow-hidden rounded-lg shadow-md" onMouseEnter={() => setActiveProject(project.id)} onMouseLeave={() => setActiveProject(null)}>
              <div className="aspect-video">
                <img src={project.image} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              </div>
              <div className={`absolute inset-0 bg-portfolio-primary/80 dark:bg-portfolio-primary/90 flex flex-col justify-end p-6 transition-opacity duration-300 ${activeProject === project.id ? 'opacity-100' : 'opacity-0 md:group-hover:opacity-100'}`}>
